...
def recipe_search(ingredient, excluded):
  ...
  app_key = ''
  result = requests.get(
    'https://api.edamam.com/search?q={}&excluded={}&app_id={}&app_key={}'.
    format(ingredient, excluded, app_id, app_key))
...
def run():
  ingredient = input(
    'Enter ingredients to search for (separated by a comma): ')
  excluded = input('Enter one ingredient to exclude (optional): ')
  calorie_check = input(
    'Would you also like to search by calories? (yes or no) ')
  if calorie_check == 'yes':
    calorie_search = True
    no_of_calories = input(
      'What is the maximum calories you want to consume (in numbers)? ')
  else:
  ...
  with open('CFG_Team_4_file.txt', 'w', encoding='utf-8') as Team4:
    ...
    for result in results:
      ...
      #ingredientLines calls a list of ingredients
      list = (recipe['ingredientLines'])
      # convert the list to a string using the repr function and display on separate lines
      ingredientlist = '\n'.join(repr(item) for item in list)

      #if calories in recipes are less than user upper limit, display recipe info
      if calorie_search == True and recipe['calories'] <= float(
          no_of_calories):
